...
| 6 ◑ | **IFDS/CFL-可达 污点（第一刀已实现 2026-06-07）** | 规模/精度 | 已落地 within-file tainted-RETURN 摘要（`summarizeReturns` + sound-leaning 规则，跨调用传播真阳、不引入误报）；完整 IFDS（参数→形参反向、跨文件、exploded graph）待续。见 [`10-interprocedural-taint.md`](./10-interprocedural-taint.md) | `extract/taint.js`、`taint.pl` | 中高 | Reps–Horwitz–Sagiv POPL'95;CFL-reachability |
