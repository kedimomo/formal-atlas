...
**✅ autoformalization "前半"——从代码 lift loop-spec 已落地（2026-06-10，迭代器界安全，零外部、零 LLM）**:`src/extract/loop/counter.js` 的 `extractLoopSpecs(fileId, code)` 用 acorn 从**真实代码**抽取循环 Hoare-spec,使 `prove <file.js | dir>` 直接作用于项目（不再只手写 spec）。**soundness 是这刀的全部**——误读循环会让 z3 "证"出不存在的代码（假信心,比不证更糟）,故识别器**极保守**:只认规范的升序计数循环 `for(let i=INIT; i</<= BOUND; i++|i+=k|i=i+k)`（INIT/BOUND 为整数字面量或标识符,k 为正整数字面量）,且**循环体内**（含闭包任意深度）**不得**重赋 counter 或 bound、**不得**含 break/continue/return/throw（直接层）、**不得**有嵌套循环;调用/条件/数组访问/对他变量赋值允许（不改整数 counter）。任何不匹配→**发 NOTHING（跳过,绝不猜）**。证的性质=**迭代器界安全**:自带不变式 `INIT<=i<=BOUND`（计数循环的机械已知不变式,故**离线放电、无需 LLM**）,z3 证 counter 不越界;off-by-one（`i<=n`）或跨步越界（`i+=2`）令该不变式非归纳→ z3 **反例驳回**（真实越界发现）。`runProveFile` 按 target 路由:`.json`→spec 模式、`.js`/目录→code 模式（`walkFiles` + 抽取 + 逐 spec `proveLoop`）。夹具 `examples/itp/loops.js`（safe `i<n` 证 ✅ / overshoot `i<=n` 驳 ❌ 反例 `i=0,n=0` / counter 重赋 + break 两个**跳过**）+ 1 测试（engines **44→45**）。**真实库实测**:`prove ../src/store/services/rebac` lift **12 个计数循环**（CSR/SpMV/Merkle 计算核心:snapshot-loader/hydration/dna-generator/immune-system/accelerator）**全 12 证迭代器界安全、0 误报、离线**;`prove ../src/server/workers` 诚实报 **0 可建模循环**（while/其他形态,保守跳过）。parity:纯增量,新 `extract/loop/` 子目录 + code 模式新分支,`.json` spec 模式/verify/refine/合成全不变（`verify sample-project` 仍 7、既有 44 测试全保）。**范围(v1 故意窄)**:仅升序整数 for 循环;降序、while、注解驱动的功能后置=后续。
