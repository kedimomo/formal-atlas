...
| **7 ◑** | **Doop 级过程间指向（首刀 + 实参流 + builtin 回调 + 字段敏感刀1 已落地）** | 规模 | Andersen points-to(`pointsto/andersen.js`)解变量持函数/动态分派 + 过程间实参流(argActual/formalParam,二阶回调) + 高阶 builtin 回调(`arr.map(cb)`,本库 +11 reaches) + **字段敏感刀1**(`field_store`/`field_call` 解 `const h={k:fn}; h[k]()`/`h.foo()` dispatch-table);behind `--points-to`、parity-safe。**诚实实测**:本库 45/67 对象字面量是 **Fastify 路由/回调注册(框架中介)**、非本地 dispatch-table → 语言级 points-to 增益有限。余(可选):上下文敏感 k-CFA、跨文件 import 变量持函数。见 [`12-points-to.md`](./12-points-to.md)、[`14-precision-engines.md`](./14-precision-engines.md) | linker、points-to、#5 引擎 | 高 | `05 §12`(完备性:给抽象补维度) |
