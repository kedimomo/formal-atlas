...
export function extractTaintJs(fileId, code) {
  ...
  for (const [fn, idxs] of paramReturns) for (const idx of idxs) facts.push(fact('param_return', `${fileId}::${fn}`, idx)) // slice-8 (QId-keyed; inert this slice, for query/explain + future cross-file)
