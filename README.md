...
| 不会写循环不变式？(★8 autoformalization) | 做不到 | `prove`（spec 缺 invariant）→ LLM **提议**、z3 **处置**（generate-and-check），离线诚实 `needs-llm` |
| 真实代码里的数组循环会越界吗？(★8 从代码 lift) | 做不到 | `prove <file.js\|dir>` → 抽**单位步长升序**计数循环（标识符 & `arr.length` 界）、z3 **证迭代器界安全**（离线、无需 LLM），off-by-one（`i<=arr.length`）给**反例**；重赋/break/变异界/step>1 等建模不了的**跳过不猜**（守"误报 0"） |
...
# 4d) ★8 autoformalization：spec 无 invariant 键 → 合成（LLM 提议 + z3 处置；离线诚实 needs-llm）
node src/cli.js prove examples/itp/sum-bound.synth.json    # 有 LLM：合成并 z3 验；无 LLM：…needs-llm + 结构化 prompt

# 4e) ★8 从代码 lift：抽真实计数循环、z3 证迭代器界安全（离线，无需 LLM）
node src/cli.js prove examples/itp/loops.js               # safe `i<n`/`i<arr.length` ✅ / off-by-one `i<=…` 反例 ❌（counter重赋/break/arr.push变异界 跳过）
