...
| 真实代码里的计数循环会越界吗？(★8 从代码 lift) | 做不到 | `prove <file.js\|dir>` → 抽升序计数循环、z3 **证迭代器界安全**（离线、无需 LLM），off-by-one/跨步越界给**反例**；建模不了的循环**跳过不猜** |
...
node src/cli.js prove examples/itp/loops.js               # safe `i<n` 证 ✅ / off-by-one `i<=n` 反例 ❌（counter重赋/break 跳过）
