...
- **大前沿推荐序 = 5 → 7 → 8**（用户问过 7→8→5;判定:**#7 需 #5 在前**——Doop 级 points-to 本质是"Datalog 跑在 Soufflé 上",上下文敏感事实爆炸 tau-prolog 扛不住,所以规模引擎 #5 是 #7 的底座;#8 最贵且依赖外部 prover,补的是已诚实的 `unchecked` 档,放最后)：
  - **#5 Soufflé / 增量 Datalog**（规模，先行）：大库 tau-prolog 慢(实测 145 文件 17s)→ Datalog→并行 C++；watch 模式增量维护。是企业级立项目的的落地前提,也是 #7 的引擎。**spec `docs/11-scale-engine.md` + 引擎已落地（2026-06-07）**：profiling 确证 solve 占 85%、瓶颈是传递闭包(`cyclic` 52.8s);**`src/verify/datalog.js` 零安装半朴素引擎** + parity 测试(★5)——真实 `store/services`(145 文件)上 `cyclic/dead_code/tainted` 与 tau-prolog **逐位一致**,引擎一趟 **33ms** 算完全部闭包 vs tau-prolog 40.9+5.9+3.6s(**110×–1238×**)。故答案是**零安装半朴素引擎(非原生 Soufflé)**。下一增量:接进 `violation/2` 热路径(物化 `r_reaches`/`tainted` + 旁路递归规则 + tau-prolog 收尾,flag-gated)。
