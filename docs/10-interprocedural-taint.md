...
> 状态：**第一刀 + 第二刀 + 第三刀 + 第四刀已实现（2026-06-07）**——within-file **tainted-RETURN 摘要**（第一刀）、within-file **param-sink / 参数→形参反向**（第二刀，带 content-type 护栏）、**跨文件 param-sink 连接**（第三刀，QId 摘要 + post-link 解析）、**跨文件 returns-taint 连接**（第四刀，QId conduit 摘要 + ret_call + post-link 注入 source）。完整 IFDS（exploded supergraph 全量 CFL-可达）仍为后续加刀。
...
## 九、后续加刀（exploded supergraph 全量 IFDS）

within-file 两刀 + cross-file param-sink + cross-file returns-taint（均含 import 别名解析）已落地。完整 IFDS 仍待：

- **多跳跨文件链**：当前第四刀的 `retTaint` 只直达文件内汇与简单转标；conduit 返回值再作为另一个跨文件 param-sink 包装器的污点实参（returns→param-sink 二跳），以及跨文件**不动点迭代**（A 调 B、B 调 C 的传递 conduit），尚未覆盖——本框架（QId 摘要 + post-link 解析）可增量承载，但需要把 `ret_call` 解析出的 source 再喂回 `taint_arg` 的发射（涉及发射/连接顺序）。
