...
2. ✅ **抽取(已落地 2026-06-07)**:`src/extract/js-ast.js` 发 `alloc(fn,fn)`+`isFunction(fn)`(函数定义即分配点)、`assign(x,y)`(`const x=y` 标识符别名)、`calleeVar(scope,v)`(标识符被调)。附加事实、对现有规则 inert(默认 verify 不变)。实测 `examples/points-to/` 单文件:`pts(h)={realHandler}` → `resolvedCall(dispatch,realHandler)` 端到端(★7 第 3 测试)。`argActual/formalParam`(过程间实参流)见 §五·5。
...
4. ☐ **夹具扩充 + 真实库实测(下一刀)**:`examples/points-to/` 加高阶/dispatch-table;`routes`/`store/services` 上量 `unresolved_call`↓、死代码 FP↓、污点经回调新真阳;**0 静态回归**(--points-to 关时位等价,已验)。跨文件(import 的变量持函数)需把 base relations 经 import_binding 解析,留后续。
   - **实测发现(2026-06-08，决策依据)**:`--points-to` 在 `routes`/`store/services` 上**当前 0 变化**——rcall(4135/3059)、dead_code(2/160)、reaches(5585/8418) 开关位等价。本库的动态分派走**方法调用**(`obj.m()` dotted)、**高阶 builtin**(`arr.map(cb)`/`.forEach`/`.then`)、**dispatch-table**(`handlers[k]()` 计算成员),**不是**首刀针对的"裸变量持函数 `const h=fn; h()`"。首刀引擎正确(夹具 `points-to`/`points-to-arg` 证)但本库无对应模式。**结论**:推动本库 FP↓ 的下一刀**不是**更多实参流精度,而是 **① 字段敏感(load/store)解 dispatch-table**(原 docs §四"第二刀")或 **② builtin-callback 建模**(`map`/`forEach`/`then` 把回调实参视为被调)。先 measure 再投精度——避免给没有对应模式的维度加成本。
5. ✅ **过程间实参流(argActual/formalParam,已落地 2026-06-08)**:`js-ast.js` 在形参循环发 `formalParam(nm,i,p)`(键到函数对象 nm=alloc 目标)、在变量调用点发 `argActual(scope,i,实参)`(仅标识符实参;Site=scope 与 `calleeVar` 对齐,故 `pipeline` 的 `calls3(file,scope,fn)` 下沉仍正确)。引擎在解析的变量调用上按 idx 配 actual↔formal 铺 assignEdge → 实参 pts 流到形参。**二阶解析**:回调经动态分派传入后,形参再被调用 `cb()` 亦解析。夹具 `examples/points-to-arg/callbacks.js`(`fn=invoke; fn(target)` → `target` 流到 `invoke` 形参 `cb` → `cb()` 解析到 `target`):`--points-to` 下 `pts(cb)={target}`、`resolvedCall={run→invoke, invoke→target}`、`reaches(invoke,target)`/`reaches(run,done)` 出现,默认关不变。★7 第 5 测试。实测 routes/auth/sample-project 默认模式**位等价**(新事实 inert,git-stash 验:routes 187 前后一致)。**首刀范围内的过程间实参流完成**;余(可选):字段/上下文敏感、跨文件 import 变量持函数。
6. ✅ **高阶 builtin 回调建模(已落地 2026-06-08,回应 §五·4 实测决策)**:`js-ast.js` 对 `X.method(cb)`（method ∈ `HIGHER_ORDER`={map,forEach,filter,reduce,flatMap,find,some,every,sort,then,catch,...}）的**裸标识符实参**发 `calleeVar(scope, arg)`——builtin 调用回调,故回调对调用者可达。引擎的 `isFunction` 闸自动滤掉非函数实参（如 `reduce(cb, seed)` 的 seed）→ 无伪边。夹具 `examples/points-to-hof/hof.js`(`users.map(formatUser)`/`users.forEach(logIt)`):`--points-to` 下 `reaches(run,formatUser)`/`reaches(run,logIt)` 出现,默认关不变。★7 第 6 测试。**真实库实测(回应 §五·4)**:`routes` 上 `--points-to` 的 `reaches` **5585→5596(+11)**——`.map`/`.forEach`/`.then` 的裸名回调被解析进调用图(`impact`/`reaches` 查询更完整);`dead_code` 不变（这些回调本就经 `addr_taken` 判活,无对应 FP）;默认模式 routes 187 位等价。`store/services` 仍 0 变化（其高阶回调多为内联箭头,非裸名）。**结论**:builtin-callback 是本库**有效**的 #7 增量（+11 真实边）;dispatch-table（`handlers[k]()`）仍需**字段敏感**（load/store,§四"第二刀",最大的剩余精度杠杆）。
