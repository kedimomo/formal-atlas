...
| 6 ◑ | **IFDS/CFL-可达 污点（第一刀+第二刀已实现 2026-06-07）** | 规模/精度 | 已落地 within-file **tainted-RETURN 摘要**（第一刀）+ **param-sink 摘要 / 参数→形参反向**（第二刀，带 content-type 护栏，调用点虚拟汇复用 ★3 `html_safe`，0 误报）；余 IFDS（跨文件摘要、exploded graph）待续。见 [`10-interprocedural-taint.md`](./10-interprocedural-taint.md) | `extract/taint{,-interproc,-patterns}.js`、`taint.pl` | 中高 | Reps–Horwitz–Sagiv POPL'95;CFL-reachability |
