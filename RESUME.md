...
> 本次会话存档点（2026-06-08，★6 **第八刀完成** → return-of-tainted-arg：param→return 透传摘要 `param_return`，与 param-sink 摘要合流——本地透传调用喂给同文件/跨文件 param-sink 都真阳，content-type 护栏跨透传成立，非透传对照 `swallow` 0 误报，真实库 routes/auth 违规计数逐位不变）。本文件 + 自动记忆（`formal-atlas-subsystem.md`）共同记录"我停在哪、下一步做什么"。
...
```bash
cd formal-atlas
npm test                                                  # 9 smoke + 33 engines(★2/★3/★4/★5/★6/★7) + MCP 16-工具自检,全绿
...
## 下一步：★1–★4 主线 + ★6 八刀已完成,余为"按需"的规模/精度工程（06-frontier-map 5–8）
- **★6 过程间污点·八刀已实现**（`docs/10-interprocedural-taint.md`）：
