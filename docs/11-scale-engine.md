...
> **① 增量闭包已搬完(add + delete,2026-06-08)**:`src/verify/closure-delta.js` 内存版——`addEdge`(祖先×后代叉积,精确) + `deleteEdge`(**局部化 DRed**:只有经 `u` 路由的源 `{u}∪ancestors(u)` 可能失去可达,对它们经更新后的 succ 做 BFS 重导,自动恢复仍有他路支撑的对,O(affected));`rebuild` 由 `closureFromEdges` 兜底。测试 ★5 两条:加边==全量(含环)、删边==全量(断环 + 他路重导 + 无操作删边)。watch 增量基元已就绪(只待 `watch.js` 接 edge-delta);CSR/SpMV/Leopard 仍按十亿边需要再上。
